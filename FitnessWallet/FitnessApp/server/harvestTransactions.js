import web3 from './web3.js';

var intervalId;

function start() {
  console.log('harvestTransactions.start');

  intervalId = Meteor.setInterval(() => {
    //console.log('harvestTransactions - harvesting');
    var patientReports = PatientReports.find({
      contractAddress : {$exists: false}
    });

    var patientPrescriptions = PatientPrescriptions.find({
      contractAddress : {$exists: false}
    });

    console.log('------');


    patientReports.forEach((patient) => {
      console.log(patient._id);
      if(patient.transactionHash) {
        var transactionReceipt = web3.eth.getTransactionReceipt(patient.transactionHash);
        console.log(transactionReceipt);
        if(transactionReceipt !== null) {
          PatientReports.update({
            _id : patient._id
          }, {
            $set : {
              contractAddress: transactionReceipt.contractAddress
            }
          })
        }
      } else {
        console.log('skipping....');
      }
    });

    patientPrescriptions.forEach((prescription) => {
      console.log(prescription._id);
      if(prescription.transactionHash) {
        var transactionReceipt = web3.eth.getTransactionReceipt(prescription.transactionHash);
        console.log(transactionReceipt);
        if(transactionReceipt !== null) {
          PatientPrescriptions.update({
            _id : prescription._id
          }, {
            $set : {
              contractAddress: transactionReceipt.contractAddress
            }
          })
        }
      } else {
        console.log('skipping....');
      }
    });


  }, 1000);
}

function end() {
}

export default {
  start: start,
  end: end
}
